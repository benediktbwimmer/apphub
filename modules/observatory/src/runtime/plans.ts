...
export function sanitizeIdentifier(value: string | null | undefined, fallback: string): string {
  const raw = typeof value === 'string' ? value.trim() : '';
  if (!raw) {
    return fallback;
  }
  const normalized = raw.replace(/[^0-9A-Za-z._-]+/g, '-').replace(/-+/g, '-').replace(/^[-.]+|[-.]+$/g, '');
  return normalized || fallback;
}

export function sanitizeFileName(value: string | null | undefined, fallbackBase: string): string {
  const raw = typeof value === 'string' ? value.trim() : '';
  if (!raw) {
...
export function normalizeMinuteKey(value: string | null | undefined): string | null {
  ...
  if (minuteKeyRegex.test(trimmed)) {
    return trimmed;
  }
  ...
  const date = new Date(candidate.endsWith('Z') ? candidate : `${candidate}:00Z`);
...
export function toEffectiveMinuteKey(effectiveAtIso: string): string {
...
export function normalizePlanPath(prefix: string | null | undefined, fileName: string): string {
  const normalizedPrefix = (prefix ?? '').replace(/^\/+|\/+$/g, '');
  const normalizedFile = fileName.replace(/^\/+/, '');
  return normalizedPrefix ? `${normalizedPrefix}/${normalizedFile}` : normalizedFile;
}

export function deriveMetastoreRecordKey(planId: string): string {
  return sanitizeIdentifier(planId, planId);
}

export function buildPlanStorage(options: {
  planId: string;
  planPath: string;
  plansPrefix?: string | null;
  includeMetastore?: boolean;
  metastoreNamespace?: string | null;
}): CalibrationPlanStorage {
  const storage: CalibrationPlanStorage = {
    plansPrefix: options.plansPrefix ?? undefined,
    planPath: options.planPath,
    nodeId: undefined
  };
  if (options.includeMetastore) {
    storage.metastore = {
      namespace: options.metastoreNamespace ?? 'observatory.reprocess.plans',
      recordKey: deriveMetastoreRecordKey(options.planId)
    };
  }
  return storage;
}
