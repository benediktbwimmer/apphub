...
import type { MetastoreCapability, SearchRecordsResult } from '@apphub/module-sdk';
import { z } from 'zod';
...
export async function lookupCalibration(
  ...
  const searchResult: SearchRecordsResult = await config.metastore.searchRecords({
...
export async function fetchCalibrationById(
  ...
  const record = await config.metastore.getRecord({
    key: trimmedId,
    principal: config.principal
  });
