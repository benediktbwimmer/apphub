...
function setupService(options?: SetupOptions) {
  ...
  const createRecord = mock.fn(
    impls.createRecord ?? (async () => ({ record, created: true }))
  );
  const upsertRecord = mock.fn(
    impls.upsertRecord ?? (async () => ({ record, created: true }))
  );
  const patchRecord = mock.fn(impls.patchRecord ?? (async () => record));
  const softDeleteRecord = mock.fn(
    impls.softDeleteRecord
      ?? (async () => ({ ...record, deletedAt: new Date('2023-01-02T00:00:00.000Z') }))
  );
  const hardDeleteRecord = mock.fn(impls.hardDeleteRecord ?? (async () => record));
  ...
  const restoreRecordFromAudit = mock.fn(impls.restoreRecordFromAudit ?? (async () => record));
...
test('createRecord skips event emission when record already exists without deletion', async () => {
  ...
  const ctx = setupService({
    record,
    implementations: {
      createRecord: async () => ({ record, created: false })
    }
  });
...
test('createRecord fails when existing record is soft deleted', async () => {
  ...
  const ctx = setupService({
    ...
    implementations: {
      createRecord: async () => ({ record: deletedRecord, created: false })
    }
  });
...
test('upsertRecord emits updated event when record already exists', async () => {
  ...
  const ctx = setupService({
    record,
    implementations: {
      upsertRecord: async () => ({ record, created: false })
    }
  });
  ...
  const ctx = setupService({
    implementations: {
      upsertRecord: async (_client, input) => ({
        record: makeRecord({ key: (input as { key: string }).key, metadata: (input as { metadata: Record<string, unknown> }).metadata }),
        created: (input as { key: string }).key === 'new-record'
      }),
      softDeleteRecord: async () => ({
        ...makeRecord({ key: 'pipeline-1', deletedAt: new Date('2023-02-01T00:00:00.000Z') })
      })
    }
  });
...
test('bulkOperations continueOnError isolates failures', async () => {
  const ctx = setupService({
    implementations: {
      upsertRecord: async () => ({ record: null, created: false })
    }
  });
