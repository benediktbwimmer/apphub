...
const DEFAULT_LOG_LIMIT = Math.max(
  1024,
  Number(process.env.CORE_DOCKER_MAX_LOG_CHARS ?? 16384)
);
const DEFAULT_KILL_GRACE_MS = Math.max(
  1000,
  Number(process.env.CORE_DOCKER_KILL_GRACE_MS ?? 5000)
);
...
/**
 * Executes core jobs by launching Docker containers on the local host.
 *
 ...
 * specified by `CORE_DOCKER_WORKSPACE_ROOT` (falling back to the system temp
 * directory) and are removed after the container exits. Environment variable
 * `CORE_DOCKER_MAX_LOG_CHARS` limits the amount of stdout/stderr captured,
 * while `CORE_DOCKER_KILL_GRACE_MS` controls the grace period between
