...
import type { SavedCoreSearchRow } from './rowTypes';
...
async function slugExists(client: PoolClient, slug: string): Promise<boolean> {
  const { rows } = await client.query('SELECT 1 FROM saved_core_searches WHERE slug = $1 LIMIT 1', [slug]);
...
function mapRow(row: SavedCoreSearchRow): SavedSearchRecord {
...
async function fetchBySlug(
  ...
  const { rows } = await client.query<SavedCoreSearchRow>(
    'SELECT * FROM saved_core_searches WHERE slug = $1 AND owner_key = $2',
    [slug, ownerKey]
  );
  const row = rows[0];
...
export async function createSavedSearch(
  ...
  return useTransaction(async (client) => {
    ...
    const { rows } = await client.query<SavedCoreSearchRow>(
      `INSERT INTO saved_core_searches (
         id,
         slug,
         owner_key,
         owner_user_id,
...
export async function updateSavedSearch(
  ...
  return useTransaction(async (client) => {
    ...
    const { rows } = await client.query<SavedCoreSearchRow>(updateQuery, [...params, normalizedSlug, owner.key]);
    const row = rows[0];
    return row ? mapRow(row) : null;
  });
}
...
export async function deleteSavedSearch(owner: SavedSearchOwner, slug: string): Promise<boolean> {
  ...
  return useTransaction(async (client) => {
    const { rowCount } = await client.query(
      'DELETE FROM saved_core_searches WHERE slug = $1 AND owner_key = $2',
