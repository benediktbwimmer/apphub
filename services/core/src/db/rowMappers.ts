...
export function mapSavedSearchRow(row: SavedCoreSearchRow): SavedSearchRecord {
  return {
    id: row.id,
    slug: row.slug,
    name: row.name,
    ...
    category: typeof row.category === 'string' && row.category.trim().length > 0 ? row.category.trim().toLowerCase() : 'core',
