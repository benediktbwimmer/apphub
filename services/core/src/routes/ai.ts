...
export async function registerAiRoutes(app: FastifyInstance): Promise<void> {
  app.get('/ai/builder/context', async (request, reply) => {
    ...
    try {
      ...
      const jobCore = jobs.map(serializeJobDefinition);
      const serviceCore = services.map((service) => serializeService(service, null));
      const workflowCore = workflows.map(serializeWorkflowDefinition);
