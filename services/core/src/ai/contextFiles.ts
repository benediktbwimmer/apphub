...
function jobDefinitionOverview(): string {
  return [
    ...
    '',
    'Refer to `context/schemas/job-definition.json` for the full JSON Schema and `context/jobs/index.json` for the current core.'
  ].join('\n');
}
...
function workflowDefinitionOverview(): string {
  return [
    ...
    '',
    'Review `context/schemas/workflow-definition.json` and `context/workflows/index.json` for the live core.'
  ].join('\n');
}
...
function workflowWithJobsOverview(): string {
  return [
    ...
    '```',
    '',
    '- Include a dependency entry for every workflow job step. Use `existing-job` for core jobs that can be referenced immediately.',
...
function buildJobCore(jobs: ReadonlyArray<JobSummary>) {
  return jobs.map((job) => ({
    slug: job.slug,
    name: job.name,
    type: job.type,
...
function buildJobCoreMarkdown(jobs: ReadonlyArray<JobSummary>): string {
  if (jobs.length === 0) {
    return [
      '# Job Core Overview',
      '',
      'No job definitions are registered in the core yet.'
    ].join('\n');
  }
  ...
  if (jobs.length > sampleLimit) {
    lines.push('', `_… ${jobs.length - sampleLimit} additional jobs omitted. See the JSON core for the full list._`);
  }

  return lines.join('\n');
}

function buildServiceCore(services: ReadonlyArray<ServiceSummary>) {
...
function buildServiceCoreMarkdown(services: ReadonlyArray<ServiceSummary>): string {
  if (services.length === 0) {
    return [
      '# Service Core Overview',
      '',
      'No services are registered in the core yet.'
    ].join('\n');
  }
  ...
  if (services.length > sampleLimit) {
    lines.push('', `_… ${services.length - sampleLimit} additional services omitted. Consult the JSON core for the remainder._`);
  }

  return lines.join('\n');
}

function buildWorkflowCore(workflows: ReadonlyArray<WorkflowSummary>) {
...
function buildWorkflowCoreMarkdown(workflows: ReadonlyArray<WorkflowSummary>): string {
  if (workflows.length === 0) {
    return [
      '# Workflow Core Overview',
      '',
      'No workflows are registered in the core yet.'
    ].join('\n');
  }
  ...
  const lines: string[] = [
    '# Workflow Core Overview',
    '',
    'The full workflow core is available at `context/workflows/index.json`. Selected entries are summarized below.'
  ];
...
function buildServiceFiles(services: ReadonlyArray<ServiceSummary>): CodexContextFile[] {
  ...
  const core = buildServiceCore(services);
  files.push({ path: 'context/services/index.json', contents: stringifyJson(core) });
  files.push({ path: 'context/services/README.md', contents: `${buildServiceCoreMarkdown(services)}\n` });
...
function buildBundleCore(bundles: ReadonlyArray<AiBundleContext>) {
...
function buildBundleCoreMarkdown(bundles: ReadonlyArray<AiBundleContext>): string {
  if (bundles.length === 0) {
    return [
      '# Bundle Core Overview',
      '',
      'No job bundles are in use by the core jobs.'
    ].join('\n');
  }
  ...
  const lines: string[] = [
    '# Bundle Core Overview',
    '',
    'Bundles described below are referenced by the current job core. Each directory under `context/bundles/` contains the manifest, metadata, and source files for the bundle.'
  ];
...
function buildBundleFiles(bundles: ReadonlyArray<AiBundleContext>): CodexContextFile[] {
  const files: CodexContextFile[] = [];
  const core = buildBundleCore(bundles);
  files.push({ path: 'context/bundles/index.json', contents: stringifyJson(core) });
  files.push({ path: 'context/bundles/README.md', contents: `${buildBundleCoreMarkdown(bundles)}\n` });
...
export function buildCodexContextFiles(options: BuildCodexContextOptions): CodexContextFile[] {
  ...
  const files: CodexContextFile[] = [
    ...
    { path: 'context/jobs/index.json', contents: stringifyJson(buildJobCore(jobs)) },
    { path: 'context/jobs/README.md', contents: `${buildJobCoreMarkdown(jobs)}\n` },
    { path: 'context/workflows/index.json', contents: stringifyJson(buildWorkflowCore(workflows)) },
    { path: 'context/workflows/README.md', contents: `${buildWorkflowCoreMarkdown(workflows)}\n` }
  ];
  ...
  // All schemas and indices are helpful regardless of mode today, but future modes could filter here.
  return files;
}
