...
async function main() {
  ...
  const jsonSchema = zodToJsonSchema(exampleConfigDescriptorSchema, 'ExampleConfigDescriptor', {
    target: 'jsonSchema7'
  });

  await fs.mkdir(path.dirname(outputPath), { recursive: true });
  await fs.writeFile(outputPath, `${JSON.stringify(jsonSchema, null, 2)}\n`, 'utf8');
  console.log(`Wrote example config schema to ${path.relative(repoRoot, outputPath)}`);
}
