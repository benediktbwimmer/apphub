...
async function withCoreEnvironment<T>(
  fn: (context: CoreTestContext) => Promise<T>,
  options: CoreStartOptions = {}
) {
  const context = await startCore(options);
  try {
...
async function testSyntheticRepositoryFlow() {
  await withCoreEnvironment(async ({ baseUrl }) => {
...
async function testRealRepositoryLaunchFlow() {
  await withCoreEnvironment(async ({ baseUrl, env }) => {
