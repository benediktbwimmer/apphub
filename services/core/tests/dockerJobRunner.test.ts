...
describe('DockerJobRunner', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.CORE_DOCKER_ENFORCE_NETWORK_ISOLATION = 'true';
    ...
    delete process.env.CORE_DOCKER_PERSIST_LOG_TAIL;
    delete process.env.CORE_DOCKER_MAX_WORKSPACE_BYTES;
  ...
  test('omits log tail from context when persistence disabled', async () => {
    process.env.CORE_DOCKER_PERSIST_LOG_TAIL = 'false';
    clearDockerRuntimeConfigCache();
  ...
  test('honours bridge network mode when overrides enabled', async () => {
    ...
    process.env.CORE_DOCKER_DEFAULT_NETWORK_MODE = 'bridge';
    clearDockerRuntimeConfigCache();
  ...
  test('fails when workspace inputs exceed configured limit', async () => {
    process.env.CORE_DOCKER_MAX_WORKSPACE_BYTES = '64';
    clearDockerRuntimeConfigCache();
