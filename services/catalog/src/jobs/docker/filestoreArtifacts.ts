...
const MAX_DOWNLOAD_BYTES = parseLimit('CATALOG_DOCKER_FILESTORE_MAX_DOWNLOAD_BYTES', DEFAULT_MAX_TRANSFER_BYTES);
const MAX_UPLOAD_BYTES = parseLimit('CATALOG_DOCKER_FILESTORE_MAX_UPLOAD_BYTES', DEFAULT_MAX_TRANSFER_BYTES);
const MAX_FILE_BYTES = parseLimit('CATALOG_DOCKER_FILESTORE_MAX_FILE_BYTES', DEFAULT_MAX_FILE_BYTES);
  ...
  for (const descriptor of outputs) {
    ...
    if (isReadableFile(stats)) {
      ...
      const response = await client.uploadFile({
        ...
        metadata: {
          uploadedBy: 'catalog.docker-runner',
          jobRunId: run.id,
          jobSlug: definition.slug,
          jobDefinitionId: definition.id,
          attempt: run.attempt,
      ...
      for (const filePath of files) {
        ...
        if (!createdDirectories.has(parentDir)) {
          await client.createDirectory({
            ...
            path: parentDir,
            metadata: {
              createdBy: 'catalog.docker-runner',
              jobRunId: run.id
            }
          }).catch(() => undefined);
