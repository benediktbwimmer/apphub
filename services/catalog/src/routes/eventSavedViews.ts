...
export async function registerEventSavedViewRoutes(app: FastifyInstance): Promise<void> {
  app.get('/events/saved-views', async (request, reply) => {
  ...
  app.post('/events/saved-views', async (request, reply) => {
    const authResult = await requireOperatorScopes(request, reply, {
  ...
  app.get('/events/saved-views/:slug', async (request, reply) => {
    const authResult = await requireOperatorScopes(request, reply, {
  ...
  app.patch('/events/saved-views/:slug', async (request, reply) => {
    const authResult = await requireOperatorScopes(request, reply, {
    ...
    try {
      const owner = resolveOwner(authResult.auth);
      const record = await updateEventSavedView(owner, parseParams.data.slug, parseBody.data);
      if (!record) {
        reply.status(404);
        return { error: 'event_saved_view_not_found' };
      }
  ...
  app.delete('/events/saved-views/:slug', async (request, reply) => {
    const authResult = await requireOperatorScopes(request, reply, {
  ...
  app.post('/events/saved-views/:slug/apply', async (request, reply) => {
    const authResult = await requireOperatorScopes(request, reply, {
  ...
  app.post('/events/saved-views/:slug/share', async (request, reply) => {
    const authResult = await requireOperatorScopes(request, reply, {
