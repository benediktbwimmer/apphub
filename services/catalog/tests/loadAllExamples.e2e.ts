...
async function importServiceManifest(app: FastifyInstance): Promise<void> {
  ...
  const serviceVariables = {
    FILE_WATCH_ROOT: path.join(repoRoot, 'services/catalog/data/examples/environmental-observatory/inbox'),
    FILE_WATCH_STAGING_DIR: path.join(repoRoot, 'services/catalog/data/examples/environmental-observatory/staging'),
    FILE_WATCH_WAREHOUSE_PATH: path.join(
      repoRoot,
      'services/catalog/data/examples/environmental-observatory/warehouse/observatory.duckdb'
    ),
