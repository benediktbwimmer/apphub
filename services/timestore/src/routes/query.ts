  ...
  app.post(
    '/datasets/:datasetSlug/query',
    {
      schema: {
        ...
        body: schemaRef('DatasetQueryRequest'),
        response: {
          200: jsonResponse('DatasetQueryResponse', 'Query executed successfully.'),
          400: errorResponse('Invalid query request.'),
          401: errorResponse('Authentication is required.'),
          403: errorResponse('Caller lacks permission to query this dataset.'),
          404: errorResponse('Dataset not found.'),
          500: errorResponse('Failed to execute query.')
        }
      }
    },
    async (request, reply) => {
