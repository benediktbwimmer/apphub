...
- **APIs**: ingestion requests hit `POST /datasets/:datasetSlug/ingest`, enqueue BullMQ jobs (or run inline for tests), and on success publish new manifests plus partition metadata tied to the shared Postgres schema.
