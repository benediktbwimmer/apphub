...
import { runWorkflowOrchestration } from '../../../services/core/src/workflowOrchestrator';
import { resetDatabasePool } from '../../../services/core/src/db/client';
...
} from '../../../services/core/src/queue';
import { registerSourceEvent } from '../../../services/core/src/eventSchedulerState';
import { ingestWorkflowEvent } from '../../../services/core/src/workflowEvents';
import { processEventTriggersForEnvelope } from '../../../services/core/src/eventTriggerProcessor';
...
`, 'utf8');
  ...
  const { buildServer } = await import('@apphub/core/server');
  const app = await buildServer();
  await app.ready();
  const corePort = await findAvailablePort();
  await app.listen({ port: corePort, host: '127.0.0.1' });
  const coreBaseUrl = `http://127.0.0.1:${corePort}`;

  try {
    await fn(app, { timestore, filestore, metastore, coreBaseUrl });
  } finally {
    await app.close();
    ...
    const plannerResponse = await app.inject({
      ...
      payload: JSON.stringify({
        parameters: {
          ...
          coreBaseUrl: context.coreBaseUrl,
          coreApiToken: OPERATOR_TOKEN,
          metastoreBaseUrl: config.metastore.baseUrl,
          metastoreNamespace: 'observatory.calibrations',
          metastoreAuthToken: config.metastore.authToken,
          calibrations: [
