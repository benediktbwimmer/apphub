...
exports.EXAMPLE_SCENARIOS = void 0;
const jobs_1 = require("./jobs");
const workflows_1 = require("./workflows");
function requireWorkflow(slug) {
    const workflow = (0, workflows_1.getExampleWorkflow)(slug);
    if (!workflow) {
        throw new Error(`Unknown example workflow: ${slug}`);
    }
    return workflow.definition;
}
function requireJobBundle(slug) {
    const bundle = (0, jobs_1.getExampleJobBundle)(slug);
    if (!bundle) {
        throw new Error(`Unknown example job bundle: ${slug}`);
    }
    return bundle;
}
function jobReference(slug) {
    ...
    steps: [
        {
            ...
            retryPolicy: { maxAttempts: 2, strategy: 'fixed', initialDelayMs: 2000 }
        },
        {
            id: 'notify-watcher',
            name: 'Notify watcher',
...
exports.EXAMPLE_SCENARIOS = [
    {
        ...
        description: 'Imports the service manifest that points the file watcher at the environmental observatory inbox, staging directory, and DuckDB warehouse so new drops automatically trigger `observatory-hourly-ingest`.',
        difficulty: 'beginner',
    ...
    },
    {
        ...
        description: 'Uploads the `observatory-inbox-normalizer` bundle (0.1.0). Pair it with the hourly ingest workflow to emit `observatory.timeseries.raw` assets from inbox files.',
        difficulty: 'intermediate',
    ...
    },
    {
        ...
        description: 'Uploads the `observatory-duckdb-loader` bundle (0.1.0) to materialize `observatory.timeseries.duckdb` assets after normalization completes.',
        difficulty: 'intermediate',
    ...
    },
    {
        ...
        description: 'Imports the `observatory-hourly-ingest` workflow definition so you can replay the hourly ingestion loop and emit observatory telemetry assets.',
        difficulty: 'intermediate',
        tags: ['observatory', 'ingest'],
        docs: [
            {
    ...
    },
    {
        ...
        description: 'Imports the `observatory-daily-publication` workflow definition. It renders visualizations from DuckDB snapshots and publishes hourly status reports with linked artifacts.',
        difficulty: 'intermediate',
        tags: ['observatory', 'reporting'],
        docs: [
            {
