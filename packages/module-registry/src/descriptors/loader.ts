...
import {
  moduleConfigDescriptorSchema,
  type ModuleConfigDescriptor,
  type ModuleDescriptorManifest
} from './schema';

export type ModuleDescriptorFile = {
  configPath: string;
  directory: string;
  descriptor: ModuleConfigDescriptor;
};

export type BundleManifestReference = ModuleDescriptorManifest & {
  absolutePath: string;
};

export type WorkflowManifestReference = ModuleDescriptorManifest & {
  absolutePath: string;
};

export async function readModuleDescriptor(configPath: string): Promise<ModuleDescriptorFile> {
  const absoluteConfigPath = path.resolve(configPath);
  const contents = await fs.readFile(absoluteConfigPath, 'utf8');
  const parsed = moduleConfigDescriptorSchema.parse(JSON.parse(contents));
  return {
    configPath: absoluteConfigPath,
    directory: path.dirname(absoluteConfigPath),
    descriptor: parsed
  } satisfies ModuleDescriptorFile;
}

export function resolveBundleManifests(file: ModuleDescriptorFile): BundleManifestReference[] {
...
export function resolveWorkflowManifests(file: ModuleDescriptorFile): WorkflowManifestReference[] {
  const manifests = file.descriptor.manifests ?? [];
