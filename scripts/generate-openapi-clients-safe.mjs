...
async function ensureWorkspaceBuildSafe(workspace, outputCheckPaths = []) {
  ...
  const workspaceMap = {
    ...
    '@apphub/shared': 'packages/shared',
    '@apphub/filestore-client': 'packages/filestore-client'
  };
  
  const workspaceDir = workspaceMap[workspace];
  if (!workspaceDir) {
    console.log(`Unknown workspace ${workspace}, skipping build`);
    return;
  }
  ...
  if (workspace) {
    ...
    if (workspace === '@apphub/module-sdk') {
      ...
      const workspaceMap = {
        ...
        '@apphub/filestore': 'services/filestore',
        '@apphub/timestore': 'services/timestore'
      };
...
async function main() {
  await ensureWorkspaceBuildSafe('@apphub/module-registry', ['packages/module-registry/dist/index.js']);
  await ensureWorkspaceBuildSafe('@apphub/module-sdk', ['packages/module-sdk/dist/index.js']);
  await ensureWorkspaceBuildSafe('@apphub/shared', ['packages/shared/dist/index.js']);
