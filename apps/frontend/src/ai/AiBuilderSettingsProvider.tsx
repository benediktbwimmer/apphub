...
function parseStoredSettings(raw: string | null): StoredSettings {
  ...
  try {
    ...
    const provider = parsed.preferredProvider === 'openai' || parsed.preferredProvider === 'codex'
...
export function AiBuilderSettingsProvider({ children }: PropsWithChildren<unknown>) {
  ...
  }, [settings, setOpenAiApiKey, clearOpenAiApiKey, setPreferredProvider, setOpenAiMaxOutputTokens]);
