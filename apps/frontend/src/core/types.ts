...
type CoreSavedSearchBase = SavedSearch<string, Record<string, unknown>>;

export type SavedCoreSearch = Omit<CoreSavedSearchBase, 'statusFilters' | 'sort'> & {
  statusFilters: IngestStatus[];
  sort: SearchSort;
};

export type SavedCoreSearchCreateInput = Omit<
