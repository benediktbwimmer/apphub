...
import { useCoreSearch } from './hooks/useCoreSearch';
import { useCoreHistory } from './hooks/useCoreHistory';
import { useCoreBuilds } from './hooks/useCoreBuilds';
import { useCoreLaunches } from './hooks/useCoreLaunches';
import { useSavedCoreSearches } from './hooks/useSavedCoreSearches';
...
function CorePage({ searchSeed, onSeedApplied, savedSearchSlug, onSavedSearchApplied }: CorePageProps) {
  ...
  const savedSearches = useSavedCoreSearches();
  const history = useCoreHistory({ repositories, setGlobalError });
  const builds = useCoreBuilds({ repositories });
  const launches = useCoreLaunches({ repositories });
  ...
  const applySavedSearch = useCallback(
    async (search: SavedCoreSearch, notifyApplied = false) => {
  ...
  const handleRenameSavedSearch = useCallback(
    async (search: SavedCoreSearch, nextName: string) => {
      try {
  ...
  const handleShareSavedSearch = useCallback(
    async (search: SavedCoreSearch) => {
      const shareUrl = `${window.location.origin}/core?saved=${encodeURIComponent(search.slug)}`;
      try {
